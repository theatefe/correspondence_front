...
function DatatableBussinessInbox() {
  ...
  const getInboxList = () => {
    ...
    fetch(`http://localhost:3000/form/inbox-busyness/`, config)
      ...
      .then(result => {
        const outboxList = result.map(item => {
          ...
          var status =
            item.busyness__status == 1
              ? "درحال بررسی"
              : item.busyness__status == 2
              ? "موافقت"
              : "رد درخواست"
          return {
            id: item.busyness_id,
            requester:
              item.busyness__requester__first_name +
              " " +
              item.busyness__requester__last_name,
            status: status,
            seen: item.seen,
            type: item.busyness__type,
  ...
  return !loading ? (
    <div className="page-content">
      <div className="container-fluid">
        <Breadcrumbs
          title="فرم های ورودی"
          breadcrumbItem="فرم های اشتغال به کار"
        />
...
DatatableBussinessInbox.propTypes = {
  preGlobalFilteredRows: PropTypes.any,
}

export default DatatableBussinessInbox
