...
const TracingCorrespondence = () => {
  ...
  const getData = () => {
    ...
    fetch(`http://localhost:3000/letter/get-parent-trans/${id}/`, config)
      .then(res => res.json())
      .then(result => {
        console.log(result)
  ...
  useEffect(() => {
    // Initialize GoJS diagram when the component mounts
    function init() {
      getData()
      ...
      var bluegrad = $(go.Brush, "Linear", { 0: "#556ee6", 1: "#556ee6" })
      var greengrad = $(go.Brush, "Linear", { 0: "#556ee6", 1: "#556ee6" })
      var dropdown = $(go.Brush, "Linear", { 0: "#556ee6", 1: "#556ee6" })
      ...
      var actionTemplate = $(
        ...
        $(
          go.TextBlock,
          { position: new go.Point(21, 5) },
          { font: "14px IRANSansX, sans-serif", stroke: "white" },
          new go.Binding("text")
        ),
        $(
          go.TextBlock,
          { position: new go.Point(160, 5) },
          { font: "14px IRANSansX, sans-serif", stroke: "white" },
      ...
      myDiagram.nodeTemplate = // the default node template
        $(
          ...
          new go.Binding("shadowColor", "shadowColor").ofModel(),
          ...
          $(
            ...
            "Auto",
            { name: "BODY" },
            $(go.Shape, "RoundedRectangle", { fill: bluegrad, stroke: null }),
            $(
              go.Panel,
              "Vertical",
              { margin: 3 },
      ...
      myDiagram.nodeTemplateMap.add(
        ...
        $(
          ...
          $(
            go.TextBlock,
            { font: "10pt IRANSansX", stroke: "white" },
            new go.Binding("text")
          )
        )
      )
      ...
      myDiagram.linkTemplate = $(
        ...
        $(
          go.Shape,
          { strokeWidth: 2 },
          new go.Binding("stroke", "linkColor").ofModel()
        ),
        $(
          go.Shape, // the arrowhead
          { toArrow: "Standard", fill: null, strokeWidth: 3 },
          new go.Binding("stroke", "linkColor").ofModel()
        )
      )
    ...
    function buttonClick() {
      ...
      myDiagramDiv.style["background-color"] =
        button.innerHTML === "Dark Mode" ? "#242424" : "#ffffff"
      button.innerHTML =
        button.innerHTML === "Dark Mode" ? "Light Mode" : "Dark Mode"
      colorSwitch()
    }
    function colorSwitch() {
      myDiagram.model.commit(m => {
        m.set(
          m.modelData,
          "linkColor",
          m.modelData.linkColor === "#eeeeee" ? "#000000" : "#eeeeee"
        )
        m.set(
          m.modelData,
          "shadowColor",
          m.modelData.shadowColor === "#eeeeee" ? "#bababa" : "#2e2e2e"
        )
      }, null)
    }
    ...
    //     go.Node,
    //     "Horizontal",
    //     $(go.Shape, "RoundedRectangle", { width: 100, height: 40 }),
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumbs */}
          <Breadcrumbs title="نامه ها" breadcrumbItem="ردیابی" />
...
export default TracingCorrespondence
