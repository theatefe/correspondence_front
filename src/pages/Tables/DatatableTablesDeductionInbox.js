...
function DatatableDeductionInbox() {
  ...
  //meta title
  document.title = "فرم‌های گواهی حقوق - سامانه مکاتبات"
  ...
  const getInboxList = () => {
    ...
    fetch(`http://localhost:3000/form/inbox-deduction/`, config)
      .then(response => response.json())
      .then(result => {
        const outboxList = result.map(item => {
          var dateTime = item.deduction__createDate
          var sendDate = dateTime.slice(0, 10)
          var sendTime = dateTime.slice(11, 16)
          var reciver =
            item.deduction__requester__first_name +
            " " +
            item.deduction__requester__last_name
          var status =
            item.deduction__status == 1
              ? "درحال بررسی"
              : item.deduction__status == 2
              ? "موافقت"
              : "رد درخواست"
          return {
        ...
        Cell: ({ cell }) => (
          ...
          >
            <li>
              ...
              >
                <Link
                  to={`/deduction-detail/${cell.value}`}
  ...
  return !loading ? (
    <div className="page-content">
      <div className="container-fluid">
        <Breadcrumbs
          title="فرم های ورودی"
          breadcrumbItem="درخواست های کسرازحقوق"
        />
...
DatatableDeductionInbox.propTypes = {
  preGlobalFilteredRows: PropTypes.any,
}

export default DatatableDeductionInbox
