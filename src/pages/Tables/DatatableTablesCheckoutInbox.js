...
function DatatableCheckoutInbox() {
  ...
  const getInboxList = () => {
    ...
    fetch(`http://localhost:3000/form/inbox-checkout/`, config)
      .then(response => response.json())
      .then(result => {
        const outboxList = result.map(item => {
          var dateTime = item.checkout__createDate
          var fromDate = item.checkout__fromDate.slice(0, 10)
          var sendDate = dateTime.slice(0, 10)
          var sendTime = dateTime.slice(11, 16)
          var type =
            item.checkout__type == 1
              ? "ترک کار"
              : item.checkout__type == 2
              ? "اخراج"
              : item.checkout__type == 3
              ? "پایان قرارداد"
              : item.checkout__type == 4
              ? "بازنشستگی"
              : "استعفا"
          return {
            id: item.checkout_id,
            requester:
              item.checkout__requester__first_name +
              " " +
              item.checkout__requester__last_name,
            person:
              item.checkout__person__first_name +
              " " +
              item.checkout__person__last_name,
            fromDate: fromDate,
            type: type,
...
DatatableCheckoutInbox.propTypes = {
  preGlobalFilteredRows: PropTypes.any,
}

export default DatatableCheckoutInbox
